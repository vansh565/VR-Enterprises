...
/* ===== Product Features - Staggered Animation ===== */
function initProductFeatures() {
  ...
  if (!('IntersectionObserver' in window)) {
    ...
    features.forEach((item) => item.classList.add('visible'));
    return;
  }
...
function initFeatureHoverSound() {
  const features = document.querySelectorAll('.feature-item');
  if (!features.length) return;

  features.forEach((item) => {
    item.addEventListener('mouseenter', () => {
      // Subtle micro-interaction - you can add a small sound here if desired
      // or just use CSS transitions which we already have
    });
  });
}

/* ===== Feature Counter Animation (Optional) ===== */
function initFeatureCounters() {
  const featureItems = document.querySelectorAll('.feature-item');
  
  featureItems.forEach((item, index) => {
    // Add a small number indicator to each feature
    const icon = item.querySelector('.feature-icon');
    if (icon) {
      // You could add a small number badge if desired
      // This is optional - uncomment if you want numbers
      // const badge = document.createElement('span');
      // badge.className = 'feature-number';
...
document.addEventListener('DOMContentLoaded', () => {
  // ... your other init functions ...
  initProductFeatures();
  // initFeatureHoverSound(); // Optional
  // initFeatureCounters(); // Optional
});
<div class="feature-item">
  ...
  <div class="feature-content">
    <h3>Cold Pressed</h3>
    <p>Extracted at low temperatures to preserve natural nutrients, aroma and flavour</p>
  </div>
</div>
/* ===== Product Features - Staggered Animation ===== */
function initProductFeatures() {
  const features = document.querySelectorAll('.feature-item');
  if (!features.length) {
    console.log('No feature items found');
    return;
  }
...
// Make sure to call it in DOMContentLoaded
document.addEventListener('DOMContentLoaded', () => {
